...
# 监听模式: key = 按 SPACE 控制起止(默认); auto = 启动后常驻监听,SPACE 改为静音切换
# 兼容老开关 XIAOZHI_AUTO_LISTEN=1 等价于 LISTEN_MODE=auto
LISTEN_MODE = os.environ.get("LISTEN_MODE", "").lower() or (
    "auto" if os.environ.get("XIAOZHI_AUTO_LISTEN", "") == "1" else "key"
)
muted = False  # auto 模式下,SPC 切换临时静音(只影响上行,不动 listen_state)
    ...
    while running:
        if old_term_settings:
            ...
            if char == ' ' and not space_pressed:
                ...
                if LISTEN_MODE == "auto":
                    muted = not muted
                    print("🔇 麦克风已静音(暂停上行)" if muted else "🎙️ 麦克风已开启(恢复上行)")
                else:
    ...
    try:
        ...
        with ALSAErrorSuppressor():
            mic = audio.open(format=pyaudio.paInt16, channels=1, rate=16000,
                            input=True, input_device_index=0, frames_per_buffer=960,
        ...
        while running and aes_opus_info['session_id']:
            if listen_state == "stop":
                time.sleep(0.1)
                continue
            # auto 模式:常驻监听;muted 临时闭麦;TTS 期间暂停上行防自激
            if muted:
                time.sleep(0.1)
                continue
            if LISTEN_MODE == "auto" and tts_state in ("start", "sentence_start"):
                time.sleep(0.02)
                continue
...
def handle_hello_message(message):
    ...
    restart_audio_streams()
    # auto 模式: hello 后自动 send_listen(start),常驻监听
    if LISTEN_MODE == "auto":
        time.sleep(0.3)
        send_listen_message("start")
        print("🎤 [auto 模式] 已自动 send_listen(start)")
...
def run():
    ...
    try:
        ...
        # 服务模式: 自动建立会话（无 stdin 按 SPACE）
        if LISTEN_MODE == "auto":
            print("🤖 [auto 模式] 自动建立会话并常驻监听...")
            send_hello_message()
